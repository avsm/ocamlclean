...
let parse read =
  let read_ptr () = { old_ofs = read () ; instr_ind = -1 } in
	...
	try match opcode with
	  ...
	  | 129 -> [ Isint ]
	  | 130 -> [ Getmethod ]
	  | 131 -> let v = read () in let ptr = read_ptr () in
	      [ Beq (v, ptr) ]
	  | 132 -> let v = read () in let ptr = read_ptr () in
	      [ Bneq (v, ptr) ]
	  | 133 -> let v = read () in let ptr = read_ptr () in
	      [ Blint (v, ptr) ]
	  | 134 -> let v = read () in let ptr = read_ptr () in
	      [ Bleint (v, ptr) ]
	  | 135 -> let v = read () in let ptr = read_ptr () in
	      [ Bgtint (v, ptr) ]
	  | 136 -> let v = read () in let ptr = read_ptr () in
	      [ Bgeint (v, ptr) ]
	  | 137 -> [ Ultint ]
	  | 138 -> [ Ugeint ]
	  | 139 -> let v = read () in let ptr = read_ptr () in
	      [ Bultint (v, ptr) ]
	  | 140 -> let v = read () in let ptr = read_ptr () in
	      [ Bugeint (v, ptr) ]
	  | 141 -> let v = read () in let ofs = read () in
	      [ Getpubmet (v, ofs) ]
	  | 142 -> [ Getdynmet ]
	  | 143 -> [ Stop ]
	  | 144 -> [ Event ]
