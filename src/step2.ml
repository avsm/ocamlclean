...
let is_removable prim bc =
  match bc with
    ...
    | Pop _ | Apply _ | Pushtrap _ | Poptrap | Raise | Offsetclosure _
    | Offsetref _ | Checksignals | Event | Break
    | Setglobal _ | Setvectitem | Setstringchar | Branch _ 
...
let compute_dep code =
  ...
  let rec f i s a z =
    ...
    in
    let error () = failwith "Step2.compute_dep" in
    let test_stack =
      if not (List.mem s stacks.(i)) then
	(stacks.(i) <- s :: stacks.(i); true) else false
    in
    let test_accu =
  ...
  in
    f 0 [] (-1) 0;
    (accus, stacks, in_main, use_ind, use, used_by)
;;
...
let compute_blocked code accus stacks in_main use_ind cleanables =
  ...
  in
	...
	check accus.(i);
	List.iter
	  (fun k -> check (List.map (fun l -> List.nth l k) stacks.(i)))
	  use_ind.(i);
	if not cleanables.(i) then
	  match code.(i) with
	    | Assign n ->
		List.iter (fun l -> blocked.(List.nth l n) <- true) stacks.(i)
	    | _ -> ()
      )
    done;
    blocked
  ...
  let error () = failwith "Step2.clean_code" in
	...
	let rec f l k r =
	  if k = 0 then r else
	    match l with hd :: tl ->
	      f tl (pred k)
		(if cleanables.(hd) && not blocked.(hd) then r else r + 1)
	      | [] -> error ()
	in
	  f (List.hd stacks.(i)) n 0
      in
	if in_main.(i) then
	  if cleanables.(i) then
		...
		| Pop _ | Apply _ | Pushtrap _ | Poptrap | Raise
		| Offsetclosure _ | Offsetref _ | Checksignals | Event | Break
		| Setglobal _ | Setvectitem | Setstringchar | Branch _ 
		| Switch (_, _) | Setfield _ | Setfloatfield _
		| Blint (_, _) | Bleint (_, _) | Bgtint (_, _) | Bgeint (_, _)
		| Beq (_, _) | Bneq (_, _) | Bultint (_, _) | Bugeint (_, _)
...
let clean code prim =
  let (accus, stacks, in_main, use_ind, use, used_by) = compute_dep code in
