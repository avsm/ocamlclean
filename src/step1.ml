...
let prepare_code old_code cleanables ignore_fulls =
  ...
  in
    g adds 0 0;
    let new_cleans =
      List.map (fun (setg_ind, used_fields) -> (map.(setg_ind), used_fields))
	cleans
    in
    let remap ptr = ptr.instr_ind <- map.(ptr.instr_ind) in
      for i = 0 to old_code_size - 1 do
	match old_code.(i) with
	  ...
	  | Bultint (_,ptr) | Bugeint (_,ptr) ->
	      remap ptr;
	  | Switch (_, tab) -> Array.iter remap tab;
	  | Closurerec (_, _, ptr, tab) -> remap ptr; Array.iter remap tab;
	  | _ -> ()
      done;
      (new_code, new_cleans)
;;
...
let clean_code new_code data cleans =
  let map = Array.create (Array.length data) [||] in
  let update_new_code (setg_ind, used_fields) =
    match new_code.(setg_ind-2), new_code.(setg_ind-1), new_code.(setg_ind) with
	  ...
	  let cpt = ref 0 in
	    map.(sgn) <- Array.make mbs (-1);
	    List.iter (fun n -> map.(sgn).(n) <- 0) used_fields;
	    for i = 0 to mbs - 1 do
	      if map.(sgn).(i) = -1 then (
  ...
  let remap i bc =
    match bc with
      | Getglobalfield (n, p) ->
	  if map.(n) <> [||] then
	    new_code.(i) <- Getglobalfield (n, map.(n).(p));
      | _ -> ()
  in
    List.iter update_new_code cleans;
    Array.iteri remap new_code;
;;
